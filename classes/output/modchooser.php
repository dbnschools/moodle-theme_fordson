...
class modchooser extends chooser {
    ...
    public function __construct($course, $modules) {
        ...
        if (count($commonlyused)&& !empty($PAGE->theme->settings->modchoosercustomlabel)) {
            $sections[] = new chooser_section('commonlyused', new lang_string('modchoosercommonlyusedtitlecustom', 'theme_fordson', format_text($PAGE->theme->settings->modchoosercustomlabel, FORMAT_HTML, array('noclean' => true))) , array_map(function ($module) use ($context) {
